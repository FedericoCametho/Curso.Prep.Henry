...
function numberOfCharacters(string) {
  ...
  for (let i = 0; i < string.length ; i++){
    if (objetoFinal[string.charAt(i)] === undefined){
      objetoFinal[string.charAt(i)] = 0;
      objetoFinal[string.charAt(i)]++;
    } else {
      objetoFinal[string.charAt(i)]++;
    }
  }
...
function capicua(numero){
  ...
  var first = 0;
  var last = numString.length - 1;

  while (first < last){
    if (numString.charAt(first) === numString.charAt(last)){
      first++;
      last--;
    } else {
...
function sortArray(arr) {
  ...
  // chequear longitud de la primera y guardarla, chequar la segunda palabra y corroborar con las anteriores
